...
class PhoneBookService:
  ...
  def read_data(self):
    ...
    if choice == 1:
      ...
      return
    ins.print_info()
    self.phonebook.append(ins)
    print('[시스템] 전화번호 저장 완료')
  ...
  def remove(self):
    ...
    for i in self.phonebook:
      if remove_input in i.values():
        ...
        print('[시스템] 삭제가 완료 되었습니다.')
        return
    print('[시스템] 삭제할 대상이 없습니다.')
    ...
    for i in self.phonebook:
      if edit_input in i.values():
        for j in i.keys():
          ...
          i[j] = edit_ing
        print('[시스템] 수정이 완료 되었습니다.')
        return
    print('[시스템] 수정할 대상이 없습니다.')
