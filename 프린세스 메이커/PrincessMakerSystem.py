...
class PrincessMaker:
    ...
    def View_Stats(self):
        print()
        print('=' * 50)
        print(f'[{self.user.daughter_name}의 능력치 - Scene {self.user.current_scene}]')
    ...
    def Save_Stats(self, scene_num=None):
        """스탯 변경 및 현재 진행 신(Scene) 위치 업데이트"""
        if scene_num is not None:
            self.user.current_scene = scene_num
        self.user.stats = self.Stats
        repo.update_user_stats(self.user)
    ...
    def Choice(self, Question, Option, After_Option, Plus_Stats, Minus_Stats):
        ...
        while True:
            ...
            if answer in ['1', '2', '3', '4']:
                idx = int(answer) - 1
                print('\n\n')
                for line in After_Option[idx]:
                    self.speak(line, 2)
                self.Plus_Stats(Plus_Stats[idx])
                self.Minus_Stats(Minus_Stats[idx])
                return None
            else:
                print('잘못된 선택지 입니다.')
                continue
    ...
    def Ending1(self):
        ...
        return all(k in up_100 for k in ['Intellect', 'Dignity', 'Attractiveness', 'Morality'])

    def Ending2(self):
        down_40 = [i for i, j in self.Stats.items() if j <= 40]
        return all(k in down_40 for k in ['Intellect', 'Dignity', 'Attractiveness', 'Morality'])

    def Ending3(self):
        up_100 = [i for i, j in self.Stats.items() if j >= 100]
        return all(k in up_100 for k in ['Stamina', 'MuscularStrength', 'Tenacity'])

    def Ending4(self):
        up_100 = [i for i, j in self.Stats.items() if j >= 100]
        return 'Morality' in up_100 and 'Stamina' in up_100
